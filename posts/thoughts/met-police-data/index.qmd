...
fig = px.line(df_grouped, x='Month', y=['count', 'three_month_avg'])
...
# Number of occurrences per month by crime type
You can click on the legend to hide or show a crime type, and double-click to isolate a crime type.
```{python}
# Plot over time, by crime type, using a stacked area chart
df_grouped = df.groupby(['Month', 'Crime type']).count().reset_index()
fig = px.area(df_grouped, x='Month', y='count', color='Crime type')
fig.update_traces(line=dict(width=2), mode='lines')
fig.update_layout(
    xaxis_title='Month',
...
## Year-on-year change in crime type, per month
```{python}
...
## Year-on-year change in crime type, per month (top 3 risers and fallers)
```{python}
...
df_change['Trend']   = df_change['Crime type'].apply(lambda ct: 'Rising' if ct in top_risers else 'Decreasing')
...
## Outcomes (cummulative totals)

```{python}
# Plot distribution of outcomes, for all crimes.

# Where outcome is not available, set it to "Unreported"
df['Last outcome category'] = df['Last outcome category'].fillna('Category N/A')

df_outcomes = df[['Last outcome category']].dropna()

# Count occurrences of each outcome
df_outcomes = df_outcomes['Last outcome category'].value_counts().reset_index()
...
```{python}
# Plot over time, by outcome, using a stacked area chart
df_outcomes = df[['Month', 'Last outcome category']].dropna()
df_outcomes['count'] = 1
df_outcomes_grouped = df_outcomes.groupby(['Month', 'Last outcome category']).count().reset_index()
fig = px.area(df_outcomes_grouped, x='Month', y='count', color='Last outcome category')
fig.update_traces(line=dict(width=2), mode='lines')
fig.update_layout(
    xaxis_title='Month',
    yaxis_title='Number of Occurrences',
