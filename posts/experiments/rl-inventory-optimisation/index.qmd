...
We will be using the [M5 Forecasting - Accuracy](https://www.kaggle.com/competitions/m5-forecasting-accuracy/data) dataset from Kaggle, which contains historical sales data for various products across different stores. The dataset includes features such as sales history, promotional events, and calendar information. We will use this data to train our RL agent in a simulated environment, to make inventory decisions such as how much stock to order and when to place orders, with the aim of maximising profits while minimising stockouts and holding costs.
...
We will select a random store and product from the sales data, but we don't want to select a product that has no sales. We will filter the products to only include those that have sales on at least 75% of the days in the dataset. This will ensure that we have enough data to train our agent.
...
We need to pick a product from the tail end of the distribution, so we have a good chance for the RL agent to learn something useful, rather than attempting to train from a data set which is too sparse. Let us filter the products to only include those that have sales on at least 75% of the days in the dataset.
...
Now that we have a store and product to work with. Let us finalise the data we will be using for the experiment.
...
To develop an intuition of what we will be working with, we can visualise some of the important data points, starting with product prices.
...
We can look at the calculated demand data, which is a rolling average of the sales data. We apply a rolling average to smooth out the demand curve, and make it more stable for the RL agent to learn from. We will also plot the demand data, with event days highlighted in red.
...
We want to affect the demand data with seasonality and holiday spikes. We will do this by boosting demand on weekends and holidays.
