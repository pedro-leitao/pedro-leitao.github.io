...
If you have ever been in linguistic domains, you have probably heard the word "ontology". In the context of graph databases, an ontology is a formal representation of a set of concepts within a domain and the relationships between those concepts.

Ontologies are essentially domain blueprints: machine‑readable vocabularies that spell out what things exist (classes), how they relate (properties), and sometimes why (logical constraints). By using universal identifiers (URIs) and logic‑friendly formalisms like [:link RDF](https://en.wikipedia.org/wiki/Resource_Description_Framework) Schema or OWL, an ontology lets disparate systems talk about “Customer,” “Order,” or “Payment Term” without playing the usual data‑dictionary telephone game, where the pay‑off is tight semantic alignment.

Graph databases are the storage engines purpose‑built for this kind of highly connected information. They persist facts as nodes and edges, optimising for pointer‑chasing traversals that perform better than JOIN‑heavy queries in traditional SQL. Two dominant flavours exist: RDF triple stores (GraphDB, Neptune‑RDF) that natively ingest ontologies and run [SPARQL](https://www.ontotext.com/knowledgehub/fundamentals/what-is-sparql/), and property‑graph stores (like Neo4j or TigerGraph) that add key‑value bags to nodes/edges for analytics‑friendly operations and [:link Cypher](https://en.wikipedia.org/wiki/Cypher_(query_language))/Gremlin query languages. In a graph data-store relationships are first‑class citizens, so asking “show me every supplier two hops from a recalled component” becomes millisecond work rather than a spaghetti SQL ordeal.

Compared with relational databases, graph stores trades rigid, table‑centric schemas for schema‑late, edge‑centric flexibility. You lose some comforts like strong ACID across huge joins, but gain the ability to evolve models on the fly and surface insights hidden behind complex join queries.

In this experiment, we will explore a simple use case for a graph database using the [MovieLens](https://grouplens.org/datasets/movielens/) dataset, which contains information about movies, users, ratings, and tags. We will use [Neo4j](https://neo4j.com/) to create a graph representation of the data and perform some interesting queries. We will then use an embedding model to compute semantic embeddings for movie titles and tags, and use those embeddings to enhance our graph queries, together with implementing a simple recommendation system using collaborative filtering.
