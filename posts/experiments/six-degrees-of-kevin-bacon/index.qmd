...
In this experiment, we will explore a simple use case for a graph database using the [MovieLens](https://grouplens.org/datasets/movielens/) dataset, which contains information about movies, users, ratings, and tags.
...
```{python}
def recommend_movies(movie_title: str, num_recommendations: int = 5) -> Tuple[Optional[str], List[Dict[str, Any]]]:
    """
...
def visualize_recommendations(source_title: str, recommendations: List[Dict[str, Any]]) -> Network:
    """
    ...
    net.add_node(source_title, label=source_title, color="orange", size=25, font={'size': 16})
...
<iframe src="jurassic_park_recommendations.html" width="100%" height="600px" style="border:none;"></iframe>
...
<iframe src="world_war_recommendations.html" width="100%" height="600px" style="border:none;"></iframe>
...
def generate_recommendation_explanations(source_title: str, recommendations: List[Dict[str, Any]]) -> List[Dict[str, str]]:
    """
...
A good further exercise would be to extend the source dataset, with further relationships such as actors, directors, and genres, and to enhance the recommendation algorithm to take these into account, by using larger datasets such as the [IMDB Non-Commercial dataset](https://developer.imdb.com/non-commercial-datasets/).